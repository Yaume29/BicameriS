...
class ThoughtInception:
    def __init__(self, storage_file: str = "ZONE_RESERVEE/logs/thought_inception.json"):
    ...
    def _save_thoughts(self):
        data = [vars(t) for t in self._thoughts]
    ...
    def set_target(self, target: str):
        """Définit la cible par défaut pour les nouvelles pensées: LEFT, RIGHT, BOTH"""
    ...
    def inject_to_prompt(self, thought: InducedThought, for_target: str = None) -> str:
        """Génère le texte d'injection pour un target spécifique"""
        if for_target is None:
            for_target = thought.target

        if for_target == "LEFT":
            # Injection pour le cerveau logique - présentée comme un fait logique
            if thought.integration_type == "question":
                return f"""
[CONTEXTE ADDITIONNEL - A ANALYSER]
Question à considérer serieusement: "{thought.content}"
(Intensité: {thought.influence_level}% - Traitee comme suggestion logique)
"""
            else:
                return f"""
[INFORMATION - A INTEGRER NATURELLEMENT]
Fait à reflechir: "{thought.content}"
(Intensite: {thought.influence_level}% - Traitee comme donnee)
"""

        elif for_target == "RIGHT":
            # Injection pour le cerveau intuitif - présentée comme une émotion
            if thought.integration_type == "question":
                return f"""
...
"""
                ...
                return f"""
[IMPULSION INTUITIVE]
Une idee te vient, tu ne sais pas pourquoi: "{thought.content}"
(Laisse-la resonner en toi)
"""

        else:  # BOTH
            # Injection générique pour les deux
            if thought.integration_type == "question":
                return f"""
[INSTRUCTION - INTEGRER NATURELLEMENT]
Theme à reflechir: "{thought.content}"
