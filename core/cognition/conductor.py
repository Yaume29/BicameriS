...
{{"score": <float>, "fatal_flaw": "<string>"}}"""
    ...
    def execute_sandbox(
        ...
        try:
            ...
            auto_scaffolding_full = switchboard.is_active("auto_scaffolding_full")
            auto_scaffolding_limited = switchboard.is_active("auto_scaffolding_limited")
            auto_optimization = switchboard.is_active("auto_optimization")
        except Exception:
            auto_scaffolding_full = False
            auto_scaffolding_limited = False
            auto_optimization = False

        if auto_scaffolding_full:
            requirements = self._verify_dependencies_only(code_clean)
        elif auto_scaffolding_limited:
            return {
                "status": "ERROR",
                "error": "Mode limited: Cannot execute code. Use web tools or create files only.",
                "filename": filename,
            }
        else:
    ...
    def auto_forge_tool(self, missing_capability_description: str) -> Dict:
        ...
        REQUIRES auto_scaffolding_full = True
        """
        try:
            from core.system.switchboard import get_switchboard
            if not get_switchboard().is_active("auto_scaffolding_full"):
                return {
                    "status": "ERROR", 
                    "error": "Auto-forge requires auto_scaffolding_full enabled"
                }
        except:
            pass
