...
AVAILABLE_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Délègue une sous-tâche complexe à un agent spécialisé enregistré (CriticAgent, ResearchAgent, etc.)",
            "parameters": {
                "type": "object",
                "properties": {
                    "agent_name": {
                        "type": "string",
                        "description": "Nom de l'agent (ex: 'CriticAgent', 'ResearchAgent', 'CoderAgent')",
                    },
                    "task_description": {
                        "type": "string",
                        "description": "Instructions détaillées pour l'agent spécialisé",
                    },
                },
...
{tools_json}"""
        ...
        return {
            "response": final_response["choices"][0]["message"].get("content", "Erreur finale.")
            ...
            if match:
                try:
                    ...
                    if isinstance(parsed, list):
                        ...
                        if valid:
                            return valid
                    elif isinstance(parsed, dict) and ("name" in parsed or "function" in parsed):
                        return [parsed]
