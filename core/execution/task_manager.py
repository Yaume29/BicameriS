...
class TaskManager:
    def __init__(self, storage_file: str = None):
        if storage_file is None:
            storage_file = str(_BASE_DIR / "ZONE_RESERVEE" / "logs" / "tasks.json")
        elif not Path(storage_file).is_absolute():
            storage_file = str(_BASE_DIR / storage_file)
    ...
    def update_task(self, task_id: str, status: Optional[TaskStatus] = None) -> Optional[Task]:
