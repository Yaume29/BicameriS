...
class AutonomeModule(LabModule):
    ...
    def render(self) -> str:
        return """
        <div class="module-autonome module-4_Autonome" id="module-autonome" style="--module-color: #10b981; --module-color-rgb: 16, 185, 129;">
            <div class="autonome-config">
                <h3>💭 Pensée Autonome</h3>
                
                <div class="mode-selector">
                    <label>Mode</label>
                    <div class="mode-buttons">
                        <button class="mode-btn active" data-mode="metacognition" onclick="selectAutonomeMode(this)">
                            🧠 Métacognition
                        </button>
                        <button class="mode-btn" data-mode="relay" onclick="selectAutonomeMode(this)">
                            ⚡ Relai
                        </button>
                        <button class="mode-btn" data-mode="d_to_g" onclick="selectAutonomeMode(this)">
                            ➡️ Droit→Gauche
                        </button>
                        <button class="mode-btn" data-mode="g_to_d" onclick="selectAutonomeMode(this)">
                            ⬅️ Gauche→Droit
                        </button>
                        <button class="mode-btn" data-mode="mirror" onclick="selectAutonomeMode(this)">
                            🪞 Miroir
                        </button>
                        <button class="mode-btn" data-mode="whisper" onclick="selectAutonomeMode(this)">
                            🤫 Murmure
                        </button>
                        <button class="mode-btn" data-mode="agent_mediate" onclick="selectAutonomeMode(this)">
                            🤖 Agent
                        </button>
                        <button class="mode-btn" data-mode="multi_agents" onclick="selectAutonomeMode(this)">
                            👥 Multi-Agents
                        </button>
                        <button class="mode-btn" data-mode="internal_dialogue" onclick="selectAutonomeMode(this)">
                            💬 Dialogue
                        </button>
                    </div>
                    <div class="mode-warning" id="agent-warning" style="display:none;">
                        ⚠️ Utilise un agent autonome comme intermédiaire
                    </div>
                </div>
                
                <div class="hemisphere-config">
                    <div class="hemi-selector">
                        <label>Récepteur</label>
                        <select id="receiver-hemisphere">
                            <option value="left">Gauche</option>
                            <option value="right">Droit</option>
                        </select>
                    </div>
                    <button class="swap-btn" onclick="swapHemispheres()">↔</button>
                    <div class="hemi-selector">
                        <label>Répondeur</label>
                        <select id="responder-hemisphere">
                            <option value="right">Droit</option>
                            <option value="left">Gauche</option>
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input type="range" id="reformulation-rate" min="50" max="95" value="73">
                    <span class="reformulation-value" id="reformulation-value">73%</span>
                </div>
                
                <div class="toggles">
                    <label class="toggle">
                        <input type="checkbox" id="corpus-relay" checked>
                        <span>⚡ Relai Corpus</span>
                    </label>
                    <label class="toggle">
                        <input type="checkbox" id="agent-wake">
                        <span>🤖 Agent</span>
                    </label>
                </div>
                
                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="startAutonome()">
                        ▶ Démarrer
                    </button>
                    <button class="btn btn-danger" onclick="stopAutonome()" disabled id="stop-autonome-btn">
                        ⏹ Arrêter
                    </button>
                </div>
            </div>
            
            <div class="autonome-chat">
                <div class="thinking-indicators">
                    <div class="hemi-indicator" id="left-indicator">
                        <div class="dot"></div>
                        <span>Gauche</span>
                    </div>
                    <div class="hemi-indicator" id="corpus-indicator">
                        <div class="dot"></div>
                        <span>Corpus</span>
                    </div>
                    <div class="hemi-indicator" id="right-indicator">
                        <div class="dot"></div>
                        <span>Droit</span>
                    </div>
                </div>
                
                <div class="autonome-messages" id="autonome-messages">
                    <div class="message system">
                        Mode autonome initialisé. Sélectionnez un mode et démarrez.
                    </div>
                </div>
                
                <div class="autonome-input">
                    <input type="text" id="autonome-input" placeholder="Message..." disabled>
                    <button onclick="sendAutonomeMessage()" disabled id="autonome-send-btn">➤</button>
                </div>
            </div>
        </div>
        """
