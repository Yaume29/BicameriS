    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                scaffolding = input(f"  Auto-scaffolding [Y/N]: ").strip().upper()
                ...
                if scaffolding in ("Y", "N"):
                    manager.set("security.auto_scaffolding", scaffolding == "Y")
                if docker in ("Y", "N"):
...
Examples:
  ...
  python run_config.py --import config.json
    ...
    parser.add_argument("--import", metavar="FILE", help="Import config from JSON file")
    ...
    if args.show:
        ...
        try:
            with open(args.import, "r", encoding="utf-8") as f:
                imported = json.load(f)
            manager.config = imported
            manager.save()
            print(f"✅ Imported configuration from {args.import}")
        except Exception as e:
